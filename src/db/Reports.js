...
const gestoresRecibosProducto = async (body) => {
  const pool = await sql.connect(sqlConfig);
  const request = pool.request();
  console.log('aqui');
  const inputs = { 
    ...
    beneficiario : {type : sql.Numeric(11,0) , value : null},
    tomador : {type : sql.Numeric(11,0) , value : null},
    producto : {type : sql.NVarChar(10) , value : `${body?.producto}`},
  };

  for (const [key, param] of Object.entries(inputs)) { request.input(key, param.type, param.value); }
  let catrina = await request.execute('spRecibosProducto');

  console.log('aqui2');
  console.log(catrina.recordset);
